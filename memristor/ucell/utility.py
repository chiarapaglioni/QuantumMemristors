...
from memristor.qinfo.qinfo import dagger, dirsum, haar_sample, rhoplot, tf_multikron
from contextlib import contextmanager
from memristor.ucell.operators import op
import sys
import os
...
def opto_gen(modes, targets):
    ...
    # initialise specification container
    bms_spec = []
...
def probfid(null):
    ...
    def combi(rho, gamma):
        ...
        return 1 - tf.real(tf.linalg.trace(tf.matmul(rho, gamma))) * tf.math.pow(
            tf.real(tf.linalg.trace(tf.matmul(rho, gam))), tf.constant(2.0))
