...
class QMemristor(object):
    ...
    def call(self, input_states):
        ...
        if len(input_states) != self.tlen:
            print("Dimension mismatch: Specified time series length does not match data length {}!={}".format(self.tlen,
                                                                                                              len(input_states)))
...
class QEncoder(object):
    ...
    def _eigen_map(self, data, rand_encoding=False):
        ...
        # assert encoding is sufficient assuming binary data
        assert 2 ** in_dim <= len(
...
def eigen_encode_map(data, modes, photons, rand_encoding=False, density=True):
    ...
    assert 2 ** in_dim <= len(
        encodings), "Woah hey woah, your system size is not sufficent to encode all states: {}<{}".format(
        len(encodings), 2 ** in_dim)
    ...
    for layer_num in range(temporal_num):
        ...
        for tnum, target in enumerate(targets):
            ...
            for state_num in range(len(data)):
                if state_num % 10 == 0:
                    print(
                        "Computing {}/{}:{}/{}:{}/{} component of reservoir channel".format(state_num, len(data), tnum,
                                                                                            len(targets), layer_num,
                                                                                            temporal_num), end="\r")
...
def entanglement_gen(dim, num=100, partition=0.5, embed_dim=None):
    ...
    for i in range(num):
        ...
        ent_states[i, 0, :dim ** 2, :dim ** 2] = np.squeeze(
            haar_sample(dim=dim ** 2, num=1))  # U_ent @ ent_state @ dagger(U_ent)
